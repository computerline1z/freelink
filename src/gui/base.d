...
class Widget {
  ...
  //final void taintcheck(void delegate() dg) {
  //  if (tainted) { dg(); tainted = false; }
  // }
  abstract void draw ();
  ...
  void fullRedraw() {
    assert(area !is null, "Can't fullRedraw while area not set");
    auto backup = area; area = null;
    setRegion(backup);
  }
...
template DefaultConstructor() {
  this(typeof(this.tupleof) t) {
    foreach (id, bogus; this.tupleof)
      this.tupleof[id] = cast(typeof(bogus))t[id];
  }
}
